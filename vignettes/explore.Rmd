...
**treeheatr** supports mixed feature types.

For continuous variables/features, we can choose to either *scale* (subtract the mean and divide by the standard deviation) or *normalize* (subtract the min and divide by the max) each feature
...
Let's take a closer look at how it handles a dataset like Titanic with categorical features (`Class`, `Sex`, `Embarked`) and continuous features (others).
...
```{r eval = FALSE}
my_metrics = yardstick::metric_set(yardstick::f_meas)
```
